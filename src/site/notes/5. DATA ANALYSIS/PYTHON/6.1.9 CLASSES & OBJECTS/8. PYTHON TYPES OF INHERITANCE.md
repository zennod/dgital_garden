---
{"dg-publish":true,"permalink":"/5-data-analysis/python/6-1-9-classes-and-objects/8-python-types-of-inheritance/","noteIcon":""}
---
...
car1.start()
print(car.color)
car.turbo() # Here, car is parent class which can access the turbo() which is method in child class
```
...
- It represents that the **SUB-CHILD CLASS** of *CHILD CLASS* of the *PARENT CLASS*. 
 ...
 - **Car class cant access Ford & Ford GT methods.**
```Python
class Car:
    def __init__(self, color) -> None:
        self.color = color
...
- Car class cant access Ford & Ford GT methods.
	- Parent classes don’t get access to methods from Child classes.
